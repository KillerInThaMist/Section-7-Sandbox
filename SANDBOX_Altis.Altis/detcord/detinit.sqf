...
//[[Vehicle('s')],ropeattpoint,baglaunchpoint,vectors]
  ["rhsusf_stryker_m1132_m2_d",[-0.5,-2.1,-0.1],[-0.5,-2.1,0.1],[[0,-0.5,0.75],[0,-0.75,-0.5]]]
];
...
detAct = {
  ...
  if(isNull (_veh getVariable "det_rope")) then {
    _veh  remoteExec ["detGenRope", groupOwner _this];
  } else {
    (_veh getVariable "det_rope") remoteExec ["detDetonateRope", groupOwner _this];
